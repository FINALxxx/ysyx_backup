...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = src1 * src2;printf("%x",src1*src2));//得到乘积的低32bits
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd)=(SEXT(src1,64)*SEXT(src2,64)) >> 32; printf("LOG:%x\n",R(rd)));//可以得到有符号数乘积的高32bits
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (uint32_t)src1 >> BITS(imm, 5, 0));
