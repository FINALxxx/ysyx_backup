...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4));//由于是riscv32，不用符号拓展
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1 + src2);
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (src1 < src2));//可以不用三目运算
  ...
  INSTPAT("0100000 0???? ????? 101 ????? 00100 11", srai   , I, R(rd) = ((int32_t)src1 >> BITS(imm, 4, 0)));//riscv32下，shamt[5]==0时才有效。这里需要使用有符号数，实现算术移位
  ...
  INSTPAT("??????? ????? ????? 100 ????? 01100 11", xori   , I, R(rd) = src1 ^ imm);
  //INSTPAT("");
