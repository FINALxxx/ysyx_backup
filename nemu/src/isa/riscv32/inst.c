...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //CTE相关指令
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, csr_t* t = csr(imm); *csr(imm) = *t & ~src1; R(rd) = *t);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, csr_t* t = csr(imm); *csr(imm) = *t | ~src1; R(rd) = *t);
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, csr_t* t = csr(imm); *csr(imm) = src1;R(rd) = *t);


  //TODO: 这里是传入s->pc还是s->snpc?
  INSTPAT("0000000 00000 00000 000 00000 11100 11",	ecall  , I, s->dnpc = isa_raise_intr(R(15),s->pc));//R(15) 为 $a5);
