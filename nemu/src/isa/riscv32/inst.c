...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //跳转语句
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4;s->dnpc = s->pc+imm;call(s->pc,s->dnpc,0));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4;s->dnpc = (src1 + imm) & ~(uint32_t)1;call(s->pc,s->dnpc,(s->isa.inst.val==0x00008067)));//其实这里应该用word_t，但是由于我全用的uint32_t，所以这边就继续沿用了，以后可以回来改
